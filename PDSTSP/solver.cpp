...
Solver::Solver(Instance* instance) : instance(instance) {
    ...
    drone_only = instance->drone_only; //drone can go to them: C''
...
void Solver::run() {
    IloEnv env;
    try {
        int num_customer = num_nodes-1; //49 nodes, 48 customer
        int N_0 = num_nodes + 1; // 49 nodes 0-49
        int N_1 = num_nodes + 2; // 49 node 1-50 (50 = 0)
        ...
        IloArray<IloNumVarArray> y(env, drone_only.size()); //indexing from 1

        for (int i = 0; i < drone_only.size(); ++i) {
            y[i] = IloNumVarArray(env, num_drones);
        ...
        for (int i = 0; i < num_drones; i++) {
            IloExpr expr2(env);
            for (int j = 0; j < drone_only.size(); j++) {
